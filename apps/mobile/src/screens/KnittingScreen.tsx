...
function StitchChipBig({ stitchId, color }: { stitchId: string; color: string }) {
  ...
  if (!def) return null
  return (
    <View style={[styles.bigChip, { backgroundColor: color, borderRadius: 18 }]}>
...
export default function KnittingScreen({ projectId }: Props) {
  ...
  const notation = useMemo(() => {
    ...
    return runs.map((r) => {
      ...
      if (!def) return r.stitchId
      return r.count > 1 ? `${def.abbr}${r.count}` : def.abbr
    }).join(', ')
  }, [stitchInstances])
    ...
    <Screen>
      ...
      <View style={styles.topBar}>
        ...
        <View style={{ flex: 1, alignItems: 'center' }}>
          <Text style={{ fontFamily: fonts.mono, fontSize: 10, color: colors.inkMute, letterSpacing: 2.5, textTransform: 'uppercase' }}>
      ...
      {seq && (
        <View style={[styles.seqCard, { backgroundColor: colors.card, borderColor: colors.rule, borderRadius: radius.md, margin: spacing[4] }]}>
          <View style={{ flex: 1 }}>
            <Text style={{ fontFamily: fonts.mono, fontSize: 10, color: colors.inkMute, letterSpacing: 2, textTransform: 'uppercase' }}>
      ...
      {/* row counter */}
      <View style={[styles.counterRow, { marginHorizontal: spacing[5] }]}>
        <View style={styles.counterLeft}>
          <Text style={{ fontFamily: fonts.mono, fontSize: 11, color: colors.inkMute, letterSpacing: 2, textTransform: 'uppercase' }}>Row</Text>
        ...
        <View style={styles.counterRight}>
          <Text style={{ fontFamily: fonts.mono, fontSize: 10, color: colors.inkMute, letterSpacing: 1.5, textTransform: 'uppercase' }}>
            {Math.round(pct * 100)}%
          </Text>
      ...
      <View style={[styles.heroCard, {
        ...
        <View style={styles.heroHeader}>
          <Text style={{ fontFamily: fonts.mono, fontSize: 10, color: colors.inkMute, letterSpacing: 2, textTransform: 'uppercase' }}>
            This row · {stitchInstances.length} sts
          </Text>
        ...
        <ScrollView contentContainerStyle={styles.chipFlow}>
          {stitchInstances.map((si, i) => (
            <StitchChipBig key={i} stitchId={si.stitchId} color={colorFor(si.stitchId)}/>
          ))}
      ...
      {/* hold buttons */}
      <View style={[styles.controls, { paddingBottom: spacing[8], paddingHorizontal: spacing[4] }]}>
        <View style={styles.holdRow}>
          {/* back hold button */}
          <SmallHoldBack holdMs={holdTimeMs} onComplete={() => retreatRow(project.id)}/>
          {/* main hold button */}
          <View style={{ flex: 1 }}>
            <HoldButton
              label="Row done"
        ...
        <Text style={{ fontFamily: fonts.mono, fontSize: 10, color: colors.inkMute, letterSpacing: 1.5, marginTop: spacing[2], textAlign: 'center' }}>
...
function SmallHoldBack({ holdMs, onComplete }: { holdMs: number; onComplete?: () => void }) {
  return (
    <View style={{ width: 64 }}>
      <HoldButton
        label=""
...
const styles = StyleSheet.create({
  ...
  seqCard:     { flexDirection: 'row', alignItems: 'center', borderWidth: 1, padding: 12 },
  ...
  heroCard:    { borderWidth: 1.5, padding: 16 },
  ...
  bigChip:     { width: 76, height: 96, alignItems: 'center', justifyContent: 'center', gap: 4 },
  controls:    { gap: 0 },
  holdRow:     { flexDirection: 'row', alignItems: 'center', gap: 12 },
})
