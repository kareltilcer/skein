...
import type { Project, Sequence } from '../types/domain'
...
type ProjectStore = {
  ...
  jumpTo: (id: string, partIdx: number, seqIdx: number, rowIdx: number, repeat?: number) => void
}
...
export const useProjectStore = create<ProjectStore>()(
  persist(
    (set) => ({
      ...
      jumpTo: (id, partIdx, seqIdx, rowIdx, repeat = 1) =>
        set((s) => {
          const idx = s.projects.findIndex((p) => p.id === id)
          if (idx === -1) return s
          const p = { ...s.projects[idx] }
