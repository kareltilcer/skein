import type { Row, RowSegment, StitchInstance } from '../types/domain'

export function expandStitches(stitches: StitchInstance[]): string[] {
  const out: string[] = []
  for (const inst of stitches) {
    for (let i = 0; i < inst.count; i++) out.push(inst.stitchId)
  }
  return out
}

function collapseRuns(ids: string[]): StitchInstance[] {
  const out: StitchInstance[] = []
  for (const id of ids) {
    const last = out[out.length - 1]
    if (last && last.stitchId === id) last.count += 1
    else out.push({ stitchId: id, count: 1 })
  }
  return out
}

export function flattenSegments(segments: RowSegment[]): StitchInstance[] {
  const ids: string[] = []
  for (const seg of segments) ids.push(...expandStitches(seg.stitches))
  return collapseRuns(ids)
}

export function segmentsFromMark(
  ...
  if (lead.length > 0)   segs.push({ type: 'fixed',  stitches: collapseRuns(lead) })
  segs.push({
    type: 'repeat',
    stitches: collapseRuns(repeat),
    rule: trail.length > 0 ? { kind: 'toLast', n: trail.length } : { kind: 'toEnd' },
  })
  if (trail.length > 0)  segs.push({ type: 'fixed',  stitches: collapseRuns(trail) })
  return segs
}

function recomputeRepeatRule(segments: RowSegment[]): RowSegment[] {
  return segments.map((seg, i) => {
    if (seg.type !== 'repeat') return seg
    let trailCount = 0
    for (let j = i + 1; j < segments.length; j++) {
      const s = segments[j]!
      if (s.type !== 'fixed') continue
      for (const inst of s.stitches) trailCount += inst.count
    }
    return {
      ...seg,
...
function appendToFlat(stitches: StitchInstance[], stitchId: string): StitchInstance[] {
...
export function appendStitchPreservingSegments(row: Row, stitchId: string): Row {
  const nextStitches = appendToFlat(row.stitches, stitchId)
  if (!row.segments) return { ...row, stitches: nextStitches }

  const segs = [...row.segments]
  const lastIdx = segs.length - 1
  const last = segs[lastIdx]!
  let nextSegs: RowSegment[]
  if (last.type === 'fixed') {
    nextSegs = segs.map((s, i) =>
      i === lastIdx ? { ...s, stitches: appendToFlat(s.stitches, stitchId) } : s,
    )
  } else {
    nextSegs = [...segs, { type: 'fixed' as const, stitches: [{ stitchId, count: 1 }] }]
  }
  return { ...row, stitches: nextStitches, segments: recomputeRepeatRule(nextSegs) }
}

function popFromFlat(stitches: StitchInstance[]): StitchInstance[] {
  if (stitches.length === 0) return stitches
  const last = stitches[stitches.length - 1]!
  if (last.count > 1) return [...stitches.slice(0, -1), { ...last, count: last.count - 1 }]
  return stitches.slice(0, -1)
}

export function removeLastStitchPreservingSegments(row: Row): Row {
  if (row.stitches.length === 0) return row
  ...
  for (let i = segs.length - 1; i >= 0; i--) {
    ...
    segs[i] = { ...s, stitches: next } as RowSegment
    break
  }
